                ...
                <form action="{{ route('petugas.update', ['id' => $user->id]) }}" method="POST">
                    ...
                    <div class="modal-body container-fluid">
                        <div class="mb-3">
                            ...
                            <input autocomplete="off" required placeholder="min 8 characters" type="password" class="form-control @error('password') is-invalid @enderror" id="password" name="password" value="{{ old('password', $user->password) }}">
                            @error('password')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
